...
public class TestDynamicDataSource {
	...
	/**
	 * 使用动态切换数据源,切割点配置在mapper层
	 * <aop:pointcut id="daoOne" expression="execution(* com.luxiu.spring.mapper.*.*(..))"
	 * /> DataSourceInterceptor 拦截器切点拦截的方法是 mapper层 测试单个插入没有问题
	 */
	@Test
	 ...
	 * 使用动态切换数据源,切割点配置在mapper层
	 * <aop:pointcut id="daoOne" expression="execution(* com.luxiu.spring.mapper.*.*(..))"
	 * /> DataSourceInterceptor 拦截器切点拦截的方法是 mapper层 测试批量插入没有问题
	 */
	@Test
	...
	/**
	 * 使用动态切换数据源,切割点配置在service层 <aop:pointcut id="daoOne" expression="execution(*
	 * com.luxiu.spring.transaction.service.*.*(..))" /> DataSourceInterceptor 拦截器切点拦截的方法是
	 * service层 测试单个插入失败，手动设置数据源则成功
	 */
	@Test
