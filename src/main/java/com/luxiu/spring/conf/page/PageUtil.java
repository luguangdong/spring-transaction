...
public class PageUtil {
	 ...
	 * @return
	 *
	 * limit [m],n; m—— [m]为可选，如果填写表示skip步长，即跳过m条。 n—— 显示条数。指从第m+1条记录开始，取n条记录。 初始记录行的偏移量是
	 * 0(而不是 1),所以便于理解传入的pagerNumber必须大于等于1。 pageSize pageNumber = (pageNumber-1) *
	 * pageSize limit pageNumber,pageSize
	 */
	 ...
	 * @return
	 *
	 * rownum oracle分页参数rownum是从1开始计数的 begin= (pageNumber - 1) * pageSize + 1; end=
	 * (pageNumber * pageSize);
	 */
